...
const { Client: LanggraphSdkClient } = require("@langchain/langgraph-sdk");
...
class LanggraphClient extends BaseClient {
  ...
  async chatCompletion({ payload, onProgress, abortController = null }) {
    ...
    try {
      ...
      if (modelOptions.stream) {
        ...
        logger.debug('>>>>>>>openai.chat.completions.STREAM');

        const langgrapApiKey = this.apiKey;
        const deployment_url = baseURL;

        const assistantName = this.modelOptions.model;
        const userId = this.modelOptions.user;

        const conversationId = this.conversationId;
        const messages = modelOptions.messages; 

        // - Appel du graph sur Langgraph platform
        const clientLG = new LanggraphSdkClient({ apiUrl: deployment_url, apiKey: langgrapApiKey });

        // - Creation d'un Agent
        const assistants = await clientLG.assistants.search({
          metadata: null,
          offset: 0,
          limit: 10,
        });
        // Langgraph auto-create an assistant for each graph registered in config.
        const agent = assistants[0];
        //  const assistant = assistants.find(a => a.name=="agent");

        // - Start a new thread
        // const thread_id = uuidv4();
        const thread_id = conversationId; // pourquoi pas reprendre le même id ?
        const threadMetadata = {
          userId: userId,
          title: "conversation from librechat"
        }
        const thread = await clientLG.threads.create({ thread_id: thread_id, metadata: threadMetadata});

        // Préparation de l'historique de conversation
        // TODO transformer role: "user" en role: "human" pour la compatibilité avec Langgraph
        //const messages = [{ role: "human", content: "what's the weather in la" }];

        // - Start a streaming run
        const config = {"configurable": {  "model_name": "openai" , userId: userId }}; // le userId pour faire le lien avec le userContext (Store)
        const streamResponse = clientLG.runs.stream(
          thread["thread_id"],
          agent["assistant_id"],
          {
            config,
            input: { messages },
            streamMode: "messages-tuple" // mode by token
          }
        );
        // processing tokens as they arrives
        for await (const chunk of streamResponse) {
          //logger.debug(chunk);
          if (chunk.data && chunk.event === "messages" ) {

            const data = chunk.data[0];
            const info = chunk.data[1];

            if (data.type==='tool') {
              //logger.debug("from Tool name : %s, content : ",data.name,data.content);
              //logger.debug("info: step: %s, node:%s ",info.langgraph_step, info.langgraph_node);
      
            } else if (data.type==='ai') {
              if (data.content) {
                // sending token to front (partial response)
...
/* 
        for await (const chunk of stream) {
          ...
          this.streamHandler.handle(chunk);  // chunk.choices?.[0]?.delta.content ?? '';
...
/*         if (!UnexpectedRoleError) {
      ...
      // regular completion
        ...
        logger.debug('>>>>>>>openai.chat.completions.CREATE');
      ...
      //this.metadata = { finish_reason };
      this.metadata = { finish_reason: null };
